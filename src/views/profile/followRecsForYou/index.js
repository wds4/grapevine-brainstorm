...
const FollowRecommendationsForYou = () => {
  ...
  const url = `https://www.graperank.tech/api/neo4j/getFollowRecommendations?recommender=${pubkeyFromUrl}&recommendee=${myPubkey}`
  ...
  return (
    <>
      <center>
        <h4>
          {profile?.displayName} <span style={{ color: 'grey' }}>@{profile?.name}</span>: Follow Recommendations For You
        </h4>
        <p>
          (profiles recommended to you by {profile?.displayName}{' '}
